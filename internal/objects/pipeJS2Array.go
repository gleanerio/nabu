...
func ToJSONArray(name, bucket, prefix string, mc *minio.Client) error {
	...
	go func() {
		...
		for object := range objectCh {
			...
			// Build ID entry , was done to better support meilie and since this ID is used only to associate a record
			// back to the graph
			s := string(b.Bytes())
			...
			// I do not want a , at the end of the last element, so i write it prior to the record and skip the first recoord
			if first {
				first = false
			} else {
				_, err = pw.Write([]byte(","))
